...
# Filename:		bot_controller_2.py
# Functions:
#			[ inverse kinematics, goal callback , feedback , main ]
# Nodes:		hb_bot_2/left right and rear wheels and detected/aruco_2
...
class HBController(Node):
    def __init__(self):
	    ...
	    #   /hb_bot_2/left_wheel_force
        self.feedbacksub=self.create_subscription(Pose2D,'/detected_aruco_2',self.feedback,10)
        self.puba=self.create_publisher(Wrench,"/hb_bot_2/left_wheel_force",10)
        self.pubb=self.create_publisher(Wrench,"/hb_bot_2/right_wheel_force",10)
        self.pubc=self.create_publisher(Wrench,"/hb_bot_2/rear_wheel_force",10)
        self.get_logger().info('bot2 have started')
...
def main(args=None):
    ...
    while rclpy.ok():
            ...
            if not hb_controller.goal_flag:
               error_x=hb_controller.bot_2_x[hb_controller.index]-hb_controller.hb_x
               error_y=hb_controller.bot_2_y[hb_controller.index]-hb_controller.hb_y
               error_theta=hb_controller.bot_2_theta-hb_controller.hb_theta
               ...
               if hb_controller.index==len(hb_controller.bot_2_x)-1:
                   hb_controller.index=0
                   hb_controller.get_logger().info('bot 2 finished ')

               elif abs(error_x)<=2 and abs(error_y)<=2 and hb_controller.index<len(hb_controller.bot_2_x):
