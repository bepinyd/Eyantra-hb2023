...
class HBController(Node):
    def __init__(self):
	    ...
	    #   /hb_bot_1/left_wheel_force
        self.feedbacksub=self.create_subscription(Pose2D,'/detected_aruco_1',self.feedback,10)
        ...
        #Similar to this you can create subscribers for hb_bot_1 and hb_bot_3
    ...
    def inverse_kinematics(self, vx, vy ,w):
        ...
        # RotMat=np.array([[0,-1/rad,len1/rad],[-sp1/(2*rad),1/(2*rad),len1/rad],[sp1/(2*rad),1/(2*rad),len1/(rad)]])
        # variables=np.array([[vx],[vy],[w]])
...
def main(args=None):
    ...
    while rclpy.ok():
            ...
            if not hb_controller.goal_flag:
               error_x=hb_controller.bot_1_x[hb_controller.index]-hb_controller.hb_x
               error_y=hb_controller.bot_1_y[hb_controller.index]-hb_controller.hb_y
               error_theta=hb_controller.bot_1_theta-hb_controller.hb_theta
            ...
            #    print(hb_controller.bot_1_x)
               ...
               if hb_controller.index==len(hb_controller.bot_1_x)-1:
                   hb_controller.index=0
                   hb_controller.get_logger().info('bot 1 finished ')

            #    print(hb_controller.index)
               elif abs(error_x)<=2 and abs(error_y)<=2 and hb_controller.index<len(hb_controller.bot_1_x):
                   hb_controller.index+=1
                   print(hb_controller.index)
                #    print(hb_controller.bot_1_x[hb_controller.index],error_x,error_y)
